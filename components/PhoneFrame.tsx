...
export function PhoneFrame({
  ...
  return (
    <div className="mx-auto w-fit max-w-full">
      <div className="rounded-[2rem] border-[6px] border-surface bg-surface shadow-[0_0_0_1px_var(--color-rule-strong)]">
        <div
          className={`flex aspect-[9/19.5] flex-col overflow-hidden rounded-[1.5rem] bg-bg ${
            sizeClassName ?? "w-[360px] max-h-[85vh] min-h-[520px]"
          }`}
        >
          <div className="shrink-0 border-b border-rule bg-surface-2 px-4 py-3">{header}</div>
          <div className="flex-1 overflow-y-auto px-3 py-3">{children}</div>
          {footer && (
            <div className="shrink-0 border-t border-rule bg-surface-2 px-3 py-2">{footer}</div>
          )}
        </div>
      </div>
    </div>
